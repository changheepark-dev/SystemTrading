        ...
        for idx, code in enumerate(self.universe.keys()):
            ...
            if check_transaction_closed() and not check_table_exist(self.strategy_name, code):
                ...
                if check_transaction_closed():
                    ...
                    sql = "select max(`index`), volume from `{}`".format(code)
                    ...
                    # 장 전인지 확인후 날짜를 -1 또는 0으로 하여 20210101 형태로 지정
                    now = check_before_open()

                    # 최근 저장 일자가 오늘이 아닌지 확인
                    if last_date[0] < now or (last_date[1] == 0 and check_before_open_s()):
                        price_df = self.kiwoom.get_price_data(code)
                        # 코드를 테이블 이름으로 해서 데이터베이스에 저장
                        insert_df_to_db(self.strategy_name, code, price_df)
...
# 유니버스 실시간 체결정보 수신 등록하는 함수 =================================================================================
    def set_universe_real_time(self):
        ...
        numl = list(range(0, int(numbers / 100)))
        if numbers % 100 != 0:
            numl.append((numbers -100) / 100)
        for a in numl:
            self.kiwoom.set_real_reg(9999 - int(a), ";".join(map(str, codes.split(';')[100*a : 100*(a+1)])), fids, "1")
        ...
        while self.is_init_success:
            try:
                ...
                if not check_transaction_open():
                    print("장시간이 아니므로 {}초간 대기합니다.". format(check_open_second_gep()))
                    time.sleep(check_open_second_gep())
                    continue
                ...
                for idx, code in enumerate(self.universe.keys()):
                    ...
                    if check_buy_timing() and not endbuy:
                        endbuy = True
                        break
                    ...
                    # (3)보유 종목인지 확인
                        ...
                        # (4)접수 주문 및 보유 종목이 아니라면 매수대상인지 확인
...
# 매도대상인지 확인하는 함수 ================================================================================================
    def check_sell_signal(self, code):
        ...
        if code not in self.kiwoom.universe_realtime_transaction_info.keys():
            ...
            cprint("매도대상 확인 과정에서 아직 체결정보가 없습니다.", 'red')
            return
        ...
        today_price_data = 0
        # 계산식 + 결과
        return check_sell(df, today_price_data, purchase_price, code)
...
# 매도 주문 접수 함수 -----------------------------------------------------------------------------------------------------
    def order_sell(self, code):
        ...
        if method_sell:
            ...
            order_result = self.kiwoom.send_order('send_sell_order', '1001', 2, code, quantity, 0, '03')
...
# 매수 대상인지 확인 함수 =================================================================================================
    def check_buy_signal_and_order(self, code):
        ...
        check_buy(df, today_price_data, code)
