...
class Kiwoom(QAxWidget):
    ...
    # 로그인
        ...
        self.account_number = self.get_account_number()                                         # 로그인 + 계좌 정보 저장
    ...
    def _set_signal_slots(self):                                           # API로 보내는 요청들을 받아 올 슬롯을 등록하는 함수
        self.OnEventConnect.connect(self._login_slot)               # 로그인 시도 결과에 대한 응답을_login_slot으로 받도록 설정
        self.OnReceiveTrData.connect(self._on_receive_tr_data)      # TR의 응답 결과를 _on_receive_tr_data를 통해 받도록 설정

        self.OnReceiveMsg.connect(self._on_receive_msg)                 # TR/주문 메시지를 _on_receive_msg을 통해 받도록 설정
        self.OnReceiveChejanData.connect(self._on_chejan_slot)      # 주문 접수/체결 결과를 _on_chejan_slot을 통해 받도록 설정

        self.OnReceiveRealData.connect(self._on_receive_real_data) # 실시간 체결 데이터를 _on_receive_real_data을 통해 받도록 설정
...
# 로그인 -----------------------------------------------------------------------------------------------------------------
    ...
    def get_account_number(self):                                               # 로그인 정보       return : ACCNO 계좌정보
        tags = {"USER_NAME" : "이름", "USER_ID" : "로그인 id", "GetServerGubun" : "접속서버(1.모의)", "ACCNO" : "계좌번호"}
        for tag in tags:
            account_list = self.dynamicCall("GetLoginInfo(QString)", tag)
            account_number = account_list.split(';')[0]
            print("{} : {}".format(tags[tag], account_number))
        return account_number


# 종목 호출 --------------------------------------------------------------------------------------------------------------
    # 0: 코스피, 10: 코스닥, 3: ELW, 8: ETF, 50: KONEX, 4: 뮤추얼 펀드, 5: 신주인수권, 6: 리츠, 9: 하이얼펀드, 30: K-OTC
    def get_code_list_by_market(self, market_type):             # 종목 불러오기   ex) ['000200', '000300']
        code_list = self.dynamicCall("GetCodeListByMarket(QString)", market_type)
        code_list = code_list.split(';')[:-1]
        return code_list

    def get_master_code_name(self, code):                        # 종목명으로 변환
        code_name = self.dynamicCall("GetMasterCodeName(QString)", code)
        return code_name

# 데이터 수신 관리 ========================================================================================================
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3,
                            unused4):  # "TR조회의 응답 결과를 얻어오는 함수"
