...
def check_sell(df, today_price_data, purchase_price, code):
    ...
    ckeck1 = (df['macd'].shift(periods=2))[-1:].values > (df['macd'].shift(periods=1))[-1:].values > df[-1:]['macd'].values
...
def check_bay(df, today_price_data, code):
    ...
    stochastic = [14, 5 , 3]
    df['stFk'] = (df['close'] - df['low'].rolling(window=stochastic[0]).min()) / (df['high'].rolling(window=stochastic[0]).max() - df['low'].rolling(window=stochastic[0]).min()) * 100
    df["stFD=SK"] = df['stFk'].rolling(window=stochastic[1]).mean()


    ckeck1 =  (df['macd'].shift(periods=1))[-1:].values == min((df['macd'].shift(periods=2))[-1:].values, (df['macd'].shift(periods=1))[-1:].values, df['macd'][-1:].values)
    ...
    return (ckeck1 and ckeck2 and ckeck3)